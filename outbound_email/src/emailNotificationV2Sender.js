...
class EmailNotificationV2Sender {
	...
	async sendEmailNotification (options) {
		const { user, creator, team, stream, codemark, review, content, sender } = options;
		const codemarkOrReview = codemark || review;
		...
		const replyTo = Config.inboundEmailDisabled ? '' : `${codemarkOrReview.id}.${stream.id}.${team.id}@${Config.replyToDomain}`;
