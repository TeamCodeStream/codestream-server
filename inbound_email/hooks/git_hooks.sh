...
githook_reconfigure_sandbox() {
	[ -z "$CS_MAILIN_TOP" ] && echo "CS_MAILIN_TOP not set. Git hook won't run." && return
	local call_sandbox_config=0
	githook_module_is_modified && call_sandbox_config=1
	[ $call_sandbox_config -eq 0  -a  -f $CS_MAILIN_TOP/package.json ] && githook_file_has_changed $CS_MAILIN_TOP/package.json && call_sandbox_config=1
	[ $call_sandbox_config -eq 0  -a  -f $CS_MAILIN_TOP/package-lock.json ] && githook_file_has_changed $CS_MAILIN_TOP/package-lock.json && call_sandbox_config=1
	[ $call_sandbox_config -eq 0  -a  \( "$CS_MAILIN_YARN" == "true"  -o  "$CS_MAILIN_YARN" == "1" \) ] && githook_file_has_changed $CS_MAILIN_TOP/yarn.lock && call_sandbox_config=1
	[ $call_sandbox_config -eq 1 ] && echo "executing $CS_MAILIN_TOP/sandbox/configure-sandbox git-hook" && $CS_MAILIN_TOP/sandbox/configure-sandbox git-hook
}
