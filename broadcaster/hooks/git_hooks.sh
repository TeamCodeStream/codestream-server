...
githook_reconfigure_sandbox() {
	[ -z "$CS_BROADCASTER_TOP" ] && echo "CS_BROADCASTER_TOP not set. Git hook won't run." && return
	local call_sandbox_config=0
	githook_module_is_modified && call_sandbox_config=1
	[ $call_sandbox_config -eq 0  -a  -f $CS_BROADCASTER_TOP/package.json ] && githook_file_has_changed $CS_BROADCASTER_TOP/package.json && call_sandbox_config=1
	[ $call_sandbox_config -eq 0  -a  -f $CS_BROADCASTER_TOP/package-lock.json ] && githook_file_has_changed $CS_BROADCASTER_TOP/package-lock.json && call_sandbox_config=1
	# [ $call_sandbox_config -eq 0  -a  \( "$CS_BROADCASTER_YARN" == "true"  -o  "$CS_BROADCASTER_YARN" == "1" \) ] && githook_file_has_changed $CS_BROADCASTER_TOP/yarn.lock && call_sandbox_config=1
	[ $call_sandbox_config -eq 1 ] && echo "executing $CS_BROADCASTER_TOP/sandbox/configure-sandbox git-hook" && $CS_BROADCASTER_TOP/sandbox/configure-sandbox git-hook
}
