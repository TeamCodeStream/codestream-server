...
if [ -f $MONGO_ACCESS_FILE ]; then
	. $MONGO_ACCESS_FILE
	[ -n "$MONGO_DB" ] && export CS_BROADCASTER_MONGO_DATABASE=$MONGO_DB
	[ -n "$MONGO_URL" ] && export CS_BROADCASTER_MONGO_URL=$MONGO_URL
	# MONGO_HOST=
	# MONGO_PORT=
	# MONGO_APP_USER=
	# MONGO_APP_PASS=
...
[ -z "$CS_BROADCASTER_MONGO_DATABASE" ] && export CS_BROADCASTER_MONGO_DATABASE=codestream
# Construct the mongo URL if need be
if [ -z "$CS_BROADCASTER_MONGO_URL" ]; then
	CS_BROADCASTER_MONGO_URL="mongodb://"
	[ -n "$MONGO_APP_PASS" ] && mongo_pass=":MONGO_APP_PASS" || mongo_pass=""
	[ -n "$MONGO_APP_USER" ] && CS_BROADCASTER_MONGO_URL="${CS_BROADCASTER_MONGO_URL}${MONGO_APP_USER}${mongo_pass}@"
	CS_BROADCASTER_MONGO_URL="${CS_BROADCASTER_MONGO_URL}$MONGO_HOST"
	[ -n "$MONGO_PORT" ] && CS_BROADCASTER_MONGO_URL="${CS_BROADCASTER_MONGO_URL}:$MONGO_PORT"
	CS_BROADCASTER_MONGO_URL="${CS_BROADCASTER_MONGO_URL}/$CS_BROADCASTER_MONGO_DATABASE"
	export CS_BROADCASTER_MONGO_URL
fi
