...
let MongoUrl = process.env.CS_MESSAGER_MONGO_URL;
if (!MongoUrl) {
	if(process.env.CS_MESSAGER_MONGO_USER) {
		MongoUrl = `mongodb://${process.env.CS_MESSAGER_MONGO_USER}:${process.env.CS_MESSAGER_MONGO_PASS}@${process.env.CS_MESSAGER_MONGO_HOST}:${process.env.CS_MESSAGER_MONGO_PORT}/${process.env.CS_MESSAGER_MONGO_DATABASE}`;
	}
	else {
		MongoUrl = `mongodb://${process.env.CS_MESSAGER_MONGO_HOST}:${process.env.CS_MESSAGER_MONGO_PORT}/${process.env.CS_MESSAGER_MONGO_DATABASE}`;
	}
}
...
module.exports = {
	mongo: {
		url: MongoUrl,
		database: process.env.CS_MESSAGER_MONGO_DATABASE
	},
	logger: {
		directory: process.env.CS_MESSAGER_LOG_DIRECTORY,	// put log files in this directory
		...
		consoleOk: process.env.CS_MESSAGER_LOG_CONSOLE_OK	// also output to the console
	},
	secrets: {
