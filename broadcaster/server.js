...
const CodeStreamConfig = require('./codestream/config');

var MakeHttpsOptions = function () {
  // read in key and cert file
  const { keyfile, certfile, cafile } = CodeStreamConfig.https;
  const options = {};
  try {
    options.key = fs.readFileSync(keyfile);
  }
  catch (error) {
    throw `could not read private key file: ${keyfile}: ${error.message}`;
  }
  try {
    options.cert = fs.readFileSync(certfile);
  }
  catch (error) {
    throw `could not read certificate file ${certfile}: ${error.message}`;
  }
  if (cafile) {
    try {
      ...
      throw `could not read certificate chain file ${cafile}: ${error.message}`;
    }
  }
  return options;
};
...
var options = {
  ...
  socketChannelLimit: Number(process.env.SOCKETCLUSTER_SOCKET_CHANNEL_LIMIT) || 10000,
  ...
  // CodeStream options
  authKey: CodeStreamConfig.secrets.auth,
  protocol: 'https',
  protocolOptions: MakeHttpsOptions(),
  port: CodeStreamConfig.https.port
};
