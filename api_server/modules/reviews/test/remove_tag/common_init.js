// base class for many tests of the "PUT /review/:id/remove-tag" requests
...
class CommonInit {
	...
	init (callback) {
		...
		BoundAsync.series(this, [
			...
			this.addTag,			// add the desired tag to the review
	...
	// add the desired tag to the review
	addTag (callback) {
	...
	makeTestData (callback) {
		...
		this.path = `/reviews/${this.review.id}/remove-tag`;
		this.data = {
			tagId: this.tagId
		};
		this.expectedReview = DeepClone(this.review);
		this.expectedReview.tags = this.expectedReview.tags || [];
		const index = this.expectedReview.tags.indexOf(this.tagId);
	...
	removeTag (callback) {
		this.doApiRequest(
			{
				method: 'put',
				path: `/reviews/${this.review.id}/remove-tag`,
				data: {
					tagId: this.tagId
				},
				token: this.users[1].accessToken
