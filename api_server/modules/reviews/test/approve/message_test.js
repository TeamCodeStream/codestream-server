...
class MessageTest extends Aggregation(CodeStreamMessageTest, CommonInit) {
	...
	get description () {
		...
		return `members of the ${whichChannel} should receive a message on the ${whichChannel} channel with the review when someone approves a review in ${streamType}`;
	}
	...
	// generate the message by issuing a request to approve the review
	generateMessage (callback) {
		this.approveReview(callback);
	}
}
