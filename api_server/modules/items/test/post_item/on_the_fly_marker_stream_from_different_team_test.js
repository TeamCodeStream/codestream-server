...
class OnTheFlyMarkerStreamFromDifferentTeamTest extends MarkerStreamOnTheFlyTest {
	...
	get description () {
		return 'should return an error when attempting to create an item with a marker for an on-the-fly stream where the stream is from a different team';
	}
	...
	// form the data we'll use in creating the item
	makeItemData (callback) {
		// before forming the item data, we'll create a second repo and file-type
