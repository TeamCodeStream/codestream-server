...
class NoCommitHashWithStreamIdTest extends Markertest {
	...
	get description () {
		return 'should return an error when attempting to create an item with a marker but not providing a commit hash, when a stream ID is also specified';
	}
	...
	makeItemData (callback) {
		// remove the commit hash from the data to use in creating the item, but keep the stream ID
		super.makeItemData(() => {
