...
class GetStreamsGreaterThanTest extends GetStreamsTest {
	...
	setPath (callback) {
		// pick the ID of a stream in the middle of the streams we created, and expect all those with IDs greater than
		...
		this.path = `/streams/?teamId=${this.team._id}&gt=${pivot}`;
		callback();
	}
}
