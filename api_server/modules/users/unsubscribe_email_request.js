// base class for unsubscribe requests
...
class UnsubscribeEmailRequest extends RestfulRequest {
	...
	async process () {
		...
		try {
			...
			const op = { 
				$set: {
					[this.getPreferenceKey()]: this.getPreferenceValue(),
					modifiedAt: now
				}
			};
	...
	async handleResponse () {
		if (this.gotError) {
			...
			this.response.redirect(`${this.getErrorCallbackUrl()}?error=${errorCode}`);
		}
		else {
			this.responseData = { user: this.updateOp };
			this.response.redirect(this.getSuccessCallbackUrl());
		}
	...
	static describe () {
		return {
			tag: 'unsubscribe-TYPE-link',
			summary: 'Respond to email link to unsubscribe from TYPE emails',
			access: 'User is authenticated through the token provided.',
			description: 'Response to email link to unsubscribe from TYPE emails.',
			input: 'A "t" parameter must be present in the query parameters, interpreted as a JSONWebToken which identifies the user',
			returns: 'Redirects to /web/unsubscribe-TYPE-complete',
			publishes: 'The response data will be published on the user channel'
		};
	}
}

module.exports = UnsubscribeEmailRequest;
