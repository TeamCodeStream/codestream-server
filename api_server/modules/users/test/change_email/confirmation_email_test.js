...
class ConfirmationEmailTest extends CodeStreamMessageTest {
	...
	validateSubstitutions (message) {
		...
		Assert.equal(payload.type, 'email', 'token payload type should be email');
	...
	validateTemplateId (message) {
		Assert.equal(message.template_id, EmailConfig.changeEmailTemplateId, 'incorrect templateId');
	}
