...
class TrackIncorrectCodeTest extends CodeStreamMessageTest {
	...
	get description () {
		return 'should send a Email Confirmation Failed event for tracking purposes when a user uses an incorrect confirmation code to confirm a registration';
	}
	...
	makeData (callback) {
		...
		this.doApiRequest(
			...
			(error, response) => {
				...
				} while (newConfirmCode === user.confirmationCode);
				this.data = { 
					email: user.email,
...
module.exports = TrackIncorrectCodeTest;
