...
class CommonInit {
	...
	makeTestData (callback) {
		...
		this.message = {
			user: {
				...
				$set: {
					...
					'preferences.notificationDelivery': this.getPreferenceValue()
				},
				$version: {
		...
		this.path = `/no-auth/unsubscribe-notification?t=${this.token}`;
		this.expectedPreferences = DeepClone(this.currentUser.user.preferences || {});
		Object.assign(this.expectedPreferences, { notificationDelivery: this.getPreferenceValue(), notifications: 'involveMe' });
		callback();
	}
