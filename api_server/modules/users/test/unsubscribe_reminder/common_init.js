...
class CommonInit {
	...
	makeTestData (callback) {
		...
		this.message = {
			user: {
				...
				$set: {
					...
					'preferences.reviewReminderDelivery': false
				},
				$version: {
		...
		this.path = `/no-auth/unsubscribe-reminder?t=${this.token}`;
		this.expectedPreferences = DeepClone(this.currentUser.user.preferences || {});
		Object.assign(this.expectedPreferences, { reviewReminderDelivery: false, notifications: 'involveMe' });
		callback();
	}

	// perform the actual unsubscribe
	unsubscribeReminder (callback) {
		this.doApiRequest(
			{
				method: 'get',
				path: this.path,
