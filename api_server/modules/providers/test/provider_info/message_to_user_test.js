...
class MessageToUserTest extends Aggregation(CodeStreamMessageTest, CommonInit) {
	...
	get description () {
		return 'user should receive a message to update user\'s provider info when provider info is set';
	}
	...
	generateMessage (callback) {
		this.setProviderInfo(error => {
			if (error) { return callback(error); }
			this.message = this.setProviderInfoResponse;
			callback();
		});
	}
}
