...
class MessageTest extends Aggregation(CodeStreamMessageTest, CommonInit) {
	...
	get description () {
		...
		return `members of the ${type} should receive a message on the ${type} channel with the code error when someone resolves a code error in ${type}`;
	}
	...
	// generate the message by issuing a request to resolve the code error
	generateMessage (callback) {
		this.resolveCodeError(callback);
	}
}
