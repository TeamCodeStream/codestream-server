...
class GetCodeErrorsBeforeLastActivityTest extends GetCodeErrorsByLastActivityTest {

	get description () {
		return 'should return the correct code errors in correct order when requesting code errors for a team and before last activity';
	}
	...
	setPath (callback) {
		super.setPath(error => {
			...
			this.path = `/code-errors?teamId=${this.team.id}&byLastActivityAt=1&before=${pivot}`;
			callback();
		});
	}
}

module.exports = GetCodeErrorsBeforeLastActivityTest;
