...
class MessageTest extends Aggregation(CodeStreamMessageTest, CommonInit) {
	...
	get description () {
		...
		return `members of the ${type} should receive a message on the ${type} channel with the code error when someone follows a code error in ${type}`;
	}
	...
	// generate the message by issuing a request to follow the code error
	generateMessage (callback) {
		this.followCodeError(callback);
	}
}
