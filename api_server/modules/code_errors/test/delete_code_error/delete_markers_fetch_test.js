...
class DeleteMarkersFetchTest extends DeleteMarkersTest {
	...
	get description () {
		return 'should delete associated markers when a code error is deleted, checked by fetching the markers';
	}
	...
	verifyMarkers (callback) {
		...
		for (let i = 0; i < numMarkers; i++) {
			...
			Assert.deepStrictEqual(marker, this.expectedMarkers[i], 'fetched marker does not match');
		}
		callback();
	}
}
