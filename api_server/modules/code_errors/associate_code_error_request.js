// handle a POST /code-errors/associate request to associate a code error with a post
...
class AssociateCodeErrorRequest extends RestfulRequest {
	...
	// get any code error matching the error entity guid
	async getPostByErrorGuid () {
		const { errorGuid } = this.request.body;
		this.post = await this.data.posts.getOneByQuery(
			{ codeErrorId: errorGuid, deactivated: false },
			{ hint: PostIndexes.byCodeErrorId }
		);
		if (!this.post) {
			this.log(`Post for error guid ${errorGuid} not found`);
...
module.exports = AssociateCodeErrorRequest;
