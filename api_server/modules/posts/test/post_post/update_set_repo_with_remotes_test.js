...
class UpdateSetRepoWithRemotesTest extends UpdateMatchedRepoWithRemotesTest {
	...
	get description () {
		return 'when a code block has a stream to be created on the fly and specifies remotes that are not known for the repo, those new remotes should be added to the known remotes for the repo';
	}

	// form the data for the post we'll create in the test
	makePostData (callback) {
		// the repo ID is not added to the code block when remotes are provided,
		...
		super.makePostData(error => {
			...
			this.data.codeBlocks[0].repoId = this.repo._id;
			callback();
		});
	}
}
