...
class OnTheFlyMarkerStreamRepoNotFoundTest extends MarkerStreamOnTheFlyTest {
	...
	get description () {
		return 'should return an error when attempting to create a post and item with a marker with an on-the-fly stream with an invalid repo id';
	}
	...
	before (callback) {
		...
		super.before(error => {
			...
			this.data.item.markers[0].repoId = ObjectID();
			callback();
		});
	}
}
