...
const MarkerTestConstants = require('../nr_comment_test_constants');
...
class MarkerLocationsTest extends GetMarkersTest {
	...
	validateResponse (data) {
		...
		Assert(markerLocations.teamId === this.team.id, 'teamId does not match');
		Assert(markerLocations.streamId === this.repoStreams[0].id, 'teamId does not match');
