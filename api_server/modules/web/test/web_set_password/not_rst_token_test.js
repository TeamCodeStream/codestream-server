...
class NotRstTokenTest extends SetPasswordTest {
	...
	get description () {
		return 'should redirect to an error page when requesting a set password page with a token that is not an rst token';
	}
	...
	before (callback) {
		super.before (error => {
			...
			const token = new TokenHandler(this.apiConfig.sharedSecrets.auth).generate({email: this.currentUser.email}, 'xyz');
