'use strict';

var Get_Posts_Test = require('./get_posts_test');
var ObjectID = require('mongodb').ObjectID;

class Get_Posts_Less_Than_Test extends Get_Posts_Test {

	get description () {
		return 'should return the correct posts when requesting posts in a stream with IDs less than some value';
	}

	set_path (callback) {
		let pivot = this.my_posts[2]._id;
		this.my_posts = this.my_posts.filter(post => ObjectID(post._id) < ObjectID(pivot));
		this.path = `/posts/?team_id=${this.team._id}&stream_id=${this.stream._id}&lt=${pivot}`;
		callback();
	}
}

module.exports = Get_Posts_Less_Than_Test;
