...
class Get_Streams_Test extends CodeStream_API_Test {
	...
	create_random_repo (n, callback) {
		let options = {
			with_random_emails: 2,
			token: this.other_user_data.access_token
		};
		if (n % 2 === 1) {
			// we put every other repo on the same team
			options.team_id = this.last_team_id;
		}
		this.repo_factory.create_random_repo(
			(error, response) => {
				...
				let team_id = response.repo.team_id;
				this.users_by_team[team_id] = this.users_by_team[team_id] || [];
				this.users_by_team[team_id] = [...this.users_by_team[team_id], ...response.users];
