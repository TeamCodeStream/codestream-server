...
const US_VAULT_HOST = 'https://vault-prd1a.r10.us.nr-ops.net:8200';
const BASE_VAULT = 'containers/teams/codestream/base/codestream-server/';
...
async function main () {
	...
	const token = tokens[US_VAULT_HOST];

	const options = {
		apiVersion: 'v1', // default
		endpoint: US_VAULT_HOST,
		token,
	};
	...
	async function read (secret) {
		const vaultPath = `${BASE_VAULT}${secret}`;
		...
		console.log(`export ${secret}=${escapedValue}`);
	}
